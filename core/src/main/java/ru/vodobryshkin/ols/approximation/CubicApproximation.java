...
import ru.vodobryshkin.ols.function.ThirdDegreePolynomial;
...
public class CubicApproximation implements Scalar {
    private final List<Double> xColumn;
    private final List<Double> yColumn;

    public CubicApproximation(List<Double> xColumn, List<Double> yColumn) {
    ...
    public MathematicalFunction value() {
        ...
        Matrix<Double> matrix = new DoubleMatrix(new ArrayList<>(List.of(
                new ArrayList<>(List.of(s6x, s5x, s4x, s3x)),
                new ArrayList<>(List.of(s5x, s4x, s3x, sxx)),
                new ArrayList<>(List.of(s4x, s3x, sxx, sx)),
                new ArrayList<>(List.of(s3x, sxx, sx, n))
        )));
        List<Double> bVector = new ArrayList<>(List.of(sxxxy, sxxy, sxy, sy));
        ...
        double d = result.getLast();

        return new ThirdDegreePolynomial(a, b, c, d);
    }
